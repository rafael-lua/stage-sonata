...
function cell_controller.updateFocus(cell_focus, grid_cells, x_index, y_index)
    ...
    msg.post(
        grid_cells[x_index][y_index].instance, "set_focus", {
            focus = true
        }
    )
...
function cell_controller.resetFocus(last, grid_cells)
    msg.post(
        grid_cells[last.x][last.y].instance, "set_focus", {
            focus = false
        }
    )
...
function cell_controller.selectBlock(grid_cells, action, x_index, y_index)
    ...
    msg.post(grid_cells[selected_block.x][selected_block.y].instance, "disable_block")
    msg.post(
        selected_block.instance, "play_animation", {
            id = hashes.cell.animation[selected_block.block]
        }
    )
    grid_cells[selected_block.x][selected_block.y].props.block = nil

    return selected_block
end

function cell_controller.placeBlock(grid_cells, placement_cell, selected_block)
    GAME_STATE:pushToPlayerProp("is_moving")

    msg.post(
        grid_cells[placement_cell.x][placement_cell.y].instance, "place_block", {
            block = selected_block.block
        }
    )
    grid_cells[placement_cell.x][placement_cell.y].props.block = selected_block.block
